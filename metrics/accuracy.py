"""Summarise answer accuracy with document-level confidence intervals.

Purpose:
    Implements the Stage-M5 accuracy contract: every headline and table cell
    reports mean judged correctness plus a bootstrap 95% CI where resampling is
    performed over documents, not individual questions.

Pipeline role:
    Table builders pass cached `pipeline.orchestrator.ResultRow` objects here
    after judging. Grouping by `doc_id` preserves within-document question
    correlation during bootstrap resampling, which is the pre-registered rule
    for the paper.

Arguments:
    None. This module is import-only; callers use `accuracy_summary(rows)`.
"""
