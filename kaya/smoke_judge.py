...
Purpose:
    ...
    (`kaya/smoke_generate.py`) with the real GPT-4o-mini judge and writes the
    eight paper table CSVs. It uses no GPU and opens no PDFs; every cell must be
    a prediction-cache hit, so phase 1 has to finish first.

Pipeline role:
    Thin login-node entry point over `experiments.smoke_run.run_judge`. It runs
    online (not offline) so the OpenAI API is reachable; `OPENAI_API_KEY` is
    forwarded from the local `.env` by `kaya.kaya` for online login runs (add it
    to `secrets.forward` in `kaya/config.json`).

CLI:
    `python -m kaya.kaya run kaya/smoke_judge.py -- [--questions N] [--k N]`
...
def main(argv: list[str] | None = None) -> int:
    ...
    run_judge(config, questions, k=args.k, n_bootstrap=args.bootstrap)
    return 0
