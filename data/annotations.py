...
def load_annotations(path: str | None = None) -> Mapping[str, DocLabel]:
    ...
    Returns an empty map when the sheet does not exist yet (the manual labelling
    pass has not been run), so callers can degrade to blank labels rather than
    crash. Every row that is present is validated.
    """
