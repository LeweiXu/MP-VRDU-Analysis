...
**generate forces the in-project weight cache instead of deferring to a stray HF_HOME (2026-07-24).** A supervisor run failed with `OSError: couldn't connect to huggingface.co ... and couldn't find them in the cached files` on TL/TLV cells: their shell had `HF_HOME` exported to an external path, prestage had written the weights to the repo `.cache` (`os.environ.update`, forced), but `ensure_cache_env` used `os.environ.setdefault`, so the run read from the empty external cache and hit the dead compute-node network. Fixed the asymmetry: `ensure_cache_env` now `os.environ.update`s the cache vars (and pops the deprecated `TRANSFORMERS_CACHE`) so it pins the repo `.cache` the same way prestage does. Safe on Kaya (the runner already exports `HF_HOME`/`HF_HUB_CACHE` to that same in-project cache, so it's a no-op there) and `config.paths.hf_home` is always `ROOT/.cache`. Runbook's "what you need first" corrected: the HF token must be exported (nothing loads `.env`), the run pins its own cache so no `HF_HOME` export is needed, and `HF_HUB_OFFLINE=1` is the fix for a no-internet GPU node.

**Robustness corrected to the true +k design: gold count blocks QUESTIONS, not pages (2026-07-23).** The g5b runs had implemented "keep_top-N gold + k distractors", which was a mistake in the plan: the +k design (PROJECT_SPEC §4.1) blocks questions by their EXACT gold count and feeds ALL their gold pages plus k ranked distractors, so within a block evidence is genuinely constant and d=0 is literally the oracle condition, already generated in `g1-representation-full`. Changes: `filter_by_hop` accepts exact counts (`hop: 1/2/3` beside any/single/multi; gold-removal rules now validated as needing multi OR an exact count >= 2); `g2_robustness.yaml` runs `gold: {mode: all}` under `hop: 1/2/3` (blocks of 480/246/40 questions, 12,256 cells, was 13,248 under the wrong design); the `selection` builder blocks its robustness rows by the corpus gold count (the hop_rung precedent — the block is a property of the question, not the condition) and emits a per-block bolded oracle-d=0 baseline row loaded from the G1 cache, making the reuse exact rather than approximate; template menu and H100 runbook updated (four-spec total 51,590 cells). `docs/PIPELINE_EXTENSION_PLAN.md` removed: all eight stages are implemented, its content is superseded by these changelog entries, and the stale "do NOT work yet" framing in `template.yaml`'s header is corrected.

**CoT budgets doubled to 2048; robustness d=0 dropped as reuse; H100 runbook retargeted (2026-07-23).** (1) `g5_faithfulness.yaml`'s cot/extract_cot decode budgets go 1024 → 2048 (template menu matched): the budget is a hard cap, not a finish guarantee, and a truncated CoT cell falls back to whole-text judging with an untrustworthy score; doubling makes that rare, the `output_truncated` canary catches the rest, and the scoring policy for flagged cells is deliberately left open. (2) `g2_robustness.yaml` runs d ∈ {1,2} only (user edit, confirmed): the gold-1 clean baseline is byte-identical (same condition base) to g2_sufficiency's keep-best cells, and the all-gold baseline IS the G1 oracle ladder on the hop:multi pool, already loaded as the selection table's pivot row; the gold-2/gold-3 blocks consequently have **no same-evidence clean baseline** and their dilution slopes read d=1→2 only (stated in the spec header). (3) `H100_RUNBOOK.md` + `ops/kaya/h100_main.json` retargeted to the four planned specs in priority order (g2_sufficiency 11,456 / g2_robustness 13,248 / g5_faithfulness 26,184 / g0_interleaved 1,694 = 52,582 cells, exactly enumerated; ~5.5-8.5 days single-H100 at ~7-10 s/cell standard, 15-30 s/cell CoT, from the measured ~33 s/cell V100 rate); prestage config trimmed to the 8B + ColQwen3 + PaddleOCR-VL + MMLongBench those specs need.

**Specs reorganised to the E1-E5 taxonomy: `g<E>_<name>.yaml`, interventions under g0 (2026-07-23).** `ops/specs/` shrinks from ~27 files to 12, named by the failure-mode the run attributes (strict E-numbers per `PROJECT_SPEC.md`; user checkpoint) with everything that is a deployment/inference lever rather than an attribution under `g0_*`: `g0_parser` (E1-adjacent, but Boundary A places the parser choice in deployment), `g0_quantization`, `g0_reasoner` (scale/family runs of record + the pending thinking/llama/32B runs), `g0_resolution`, `g0_interleaved`. Attribution files: `g2_retrieval` (record + pending rerun + the locally-runnable g2-dpi sweep; `complete_retrieval` gained `--run-tag` to select a run from a multi-run file), `g2_sufficiency`/`g2_robustness` (the LOPO/distractor page_set runs, formerly g5a/g5b files), `g3_representation` (the headline ladder; its scanned half feeds E1 and its hop covariate E4), `g5_hallucination` (three-mode record), `g5_faithfulness` (six-mode re-run, both pools in one file). **Run_tags and task_names are untouched** (they are the cache identifiers; only filenames changed), so every cached tag stays judge-reachable — verified: all 11 real cached tags covered, every PROJECT_SPEC §4 planned run present, no duplicate tags, all 12 files parse. Deleted as temporary: the kaya_* per-run files (contents folded into the new groups), the jobB/jobC resume shims, `kaya_failed_rerun`, the stale `kaya_g1_parser_full` precursor (tag never cached), the unlimited-parser smoke, `g1_prompting_answerable` (superseded by the six-mode faithfulness specs), `h100_main`, `g1_failed_only_a100`, the 5070 smoke spec, and `target_template.yaml` (its vocabulary was merged into `template.yaml`).

**Extension smoke-verified end to end on the local RTX 5070 (2026-07-23).** `ops/specs/smoke_5070_extension.yaml`: four tiny runs (2B bf16, low resolution, rungs T/V so no parser env, 4 questions each, isolated `smoke-*` tags) exercising every Phase A-D mechanism with real GPU generation: 88/88 cells ok in ~3.5 min, judged with the stub judge. Verified from the rows: all six prompt modes as condition suffixes; per-cell decode-budget rebind visible in row metadata (cot=192 vs default=64) with `output_truncated` recorded on every row (17 truncated cells at the tight budget, concentrated in `none`/`extract_cot` as expected); the `run_settings.json` sidecar written, accepted by the judge, and **refusing a mismatched budget live**; page_set cells carrying the `pageset:` grammar with `provenance=constructed`, keep_top-1 feeding exactly one gold page, d=2 feeding three pages in document order, drop_top withholding exactly the top-ranked gold page (checked against the corpus), the hop:multi filter holding, and the bm25 ranking memo persisted; the retrieved+abstain path abstaining 7/16 under `abstain` vs 0 under `none`; delimiter extraction judging only the post-`Answer:` tail while the raw 498-char reasoning answer stays byte-identical on the result row (48/48 rows join-checked); and the `selection`/`faithfulness_pools` builders assembling real tables from the smoke rows. A full re-run of the spec is a pure cache hit (caches byte-identical), confirming key stability. Runs in the new `~/venvs/mpvrdu-gpu` env (torch 2.11 cu128 for Blackwell + transformers 5.14); still cluster-only: Thinking/Llama/32B weight runs, real-judge passes, and the real-anchor reconciliation checks.

**Pipeline extension Phase D: four extension builders + build-time reconciliation gate (2026-07-23).** Stages 7+8 of `docs/PIPELINE_EXTENSION_PLAN.md`. Four new builders through the normal plan path, all raising cleanly on empty input so `_safe` log-skips them until their runs land: `selection` (page_set condition × ranking source × rung, grouped purely on the `pageset:` grammar via `_common.pageset_rule`, with the all-gold G1 oracle row on the same hop=multi pool loaded builder-internally), `faithfulness_pools` (prompt mode × rung with the answerable G4 and unanswerable G3-re-run panels side by side, plus a truncated-cell count column read from `metadata.output_truncated`), `reasoner_unified` (precision / scale / matched-budget / family / reasoning-variant blocks sharing the rung columns, weight footprint not peak VRAM, per-cell n everywhere, and the Thinking block reporting **M−S** rather than pooled accuracy), and `levers` (one row per inference-time lever with per-lever sources loaded in the builder; blank rows await their runs; currently populated: resolution +2.7 TLV, abstention prompt +61.8, retrieval depth +7.2 PROVISIONAL, model swap −22.1). New `reporting/reconcile.py` + `ops.build` hook: declared anchor checks run after assembly, and a **failing check withholds the gated table, stamps all_tables.md, and exits 1** (the deliberate opposite of `_safe`, which still covers builder crashes); absent-data checks SKIP non-fatally. Anchors live in the build with provenance comments, not in tests (the fixtures policy forbids pinning real-data numbers; anchors are dataset+judge properties): the headline ladder 31.9/39.4/56.8/45.9 (checked against both `headline_summary` and `reasoner_unified`'s 8B row — 8 live PASSes today), the G4-none-reproduces-headline check (tol 1.0 for rare delimiter-echo flips), and the G3-re-run abstention anchors 16.9/10.7/80.7 (note: those are the POOLED-across-rungs rates from the hallucination table, not a TLV-only cut — the tighter TLV-only figures are 15.9/77.7). Also fixed `ops/scripts/dump_docstrings.py`'s repo-root resolution (`parents[2]`, was pointing at `ops/`); `docs/REPO_STRUCTURE.md` itself was deleted in an earlier commit and is not recreated here.
